...
function deletefile(filename) {
  if (confirm("Подтвердите удаление " + filename)) {
    msgline("Пожалуйста подождите. Идет удаление...");
    executecommand("del=" + filename);
  }
}

function renamefile(filename) {
  var newname = prompt("Новое имя для " + filename, filename);
  if (newname != null) {
    msgline("Пожалуйста подождите. Идет переименование...");
...
function editfile(filename) {
  msgline("Пожалуйста подождите. Запускается редактор...");
  ...
  editxhr.onreadystatechange = function () {
    ...
    if (this.readyState === DONE) {
      ...
      document.getElementById("o3").innerHTML = "Редактирование " + filename;
      msgline("");
    }
  };
...
function dropHandler(ev) {
  console.log('Файл(ов) сброшен(о)');
  
  globaldropfilelisthlpr = ev.dataTransfer;
  transferitem = 0;

  msgline("Пожалуйста подождите. Передача файла...");
...
function downloadall() {
  msgline("Отправить все файлы в одном zip.");
